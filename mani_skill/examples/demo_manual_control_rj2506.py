...
def parse_args():
    ...
    parser.add_argument("-r", "--robot_uids", type=str, default="rj2506")
        ...
        if logger is not None:
            for k, v in info.items():
                if isinstance(v, dict):
                    # v is an object/dict: print its keys and values
                    for kk, vv in v.items():
                        logger.add_scalar(f"{k}/{kk}", vv.float().mean(), id)
                else:
                    logger.add_scalar(f"basic/{k}", v.float().mean(), id)

            if "final_info" in info:
                final_info = info["final_info"]
                done_mask = info["_final_info"]
                for k, v in final_info["episode"].items():
                    logger.add_scalar(f"final/{k}", v[done_mask].float().mean(), id)
