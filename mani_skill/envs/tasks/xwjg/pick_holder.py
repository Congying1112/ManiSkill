...
from mani_skill.agents.robots import SO100, Fetch, Panda, WidowXAI, XArm6Robotiq, PandaWristCam
...
class PickHolderEnv(BaseEnv):
    ...
    agent: Union[Panda, SO100, PandaWristCam]
    goal_thresh = 0.025
    holder_spawn_half_size = 0.05
    holder_spawn_center = (0, 0)
    ...
    def _load_agent(self, options: dict):
        ...
        # print("new_positions", self.agent.robot.root_pose.p)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # agent_xyz=self.agent.tcp_pose.raw_pose
            # print("agent_xyz", agent_xyz)
            # agent_xyz[:, :2] += (
            #     torch.rand((b, 2)) * self.holder_spawn_half_size * 2
            #     - self.holder_spawn_half_size
            # )
            # agent_xyz[:, 0] += self.holder_spawn_center[0]
            # agent_xyz[:, 1] += self.holder_spawn_center[1]
            # agent_xyz[:, 2] = torch.rand((b)) * self.max_goal_height + xyz[:, 2]
            # self.agent.robot.set_root_pose(Pose.create_from_pq(xyz, qs))
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        ...
        if self.robot_uids in ["panda_wristcam", "panda", "widowxai"]:
            qvel = qvel[..., :-2]
...
PickHolderEnv.__doc__ = PICK_HOLDER_DOC_STRING.format(robot_id="PandaWristCam")